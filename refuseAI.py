...
start_button = st.button("練習を開始する", disabled=start_button_disabled)
# ★★★ 変更箇所 ここまで ★★★
...
# --- 6. AIからの最初の誘いを生成し表示 ---
if st.session_state.get("current_scenario") != scenario_input or not st.session_state.initial_prompt_sent:
...
def save_chat_history(history):
    if os.path.exists(CHAT_LOG_FILE):
        with open(CHAT_LOG_FILE, "r", encoding="utf-8") as f:
            try:
                logs = json.load(f)
            except json.JSONDecodeError: # ファイルが空や不正なJSONの場合
                logs = []
    else:
        logs = []
...
if not st.session_state.initial_prompt_sent:
    with st.spinner("AIが誘いを考えています..."):
        initial_response = st.session_state.genai_chat.send_message(system_prompt)
        st.session_state.chat_history.append({"role": "assistant", "content": initial_response.text})
        st.session_state.initial_prompt_sent = True
        st.rerun()
...
if not all_histories:
    ...
    # 新しい履歴が上に来るように逆順で表示
    for i, log in enumerate(reversed(all_histories)):
        # 展開可能なセクションで各セッションの履歴を表示
        with st.expander(f"セッション: {log['timestamp']} (ID: {log['session_id'][-4:]})"):
            # 各会話メッセージを表示
            for message in log["history"]:
                # システムプロンプトは表示しないようにスキップ
                if message["role"] == "assistant" and "あなたはユーザーが誘いを断る練習をするためのロールプレイング相手です。" in message["content"]:
                    continue # システムプロンプトを含むAIの最初のメッセージはスキップ
                with st.chat_message(message["role"]):
                    st.markdown(message["content"])

            # 削除ボタン
            # 同じキー名で複数のボタンがあるとエラーになるため、ユニークなkeyを指定
            if st.button(f"このセッションを削除 ({log['session_id'][-4:]})", key=f"delete_btn_{log['session_id']}"):
                delete_chat_history(log['session_id'])
